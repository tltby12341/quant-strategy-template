...
def get_project_files(main_file_path):
    ...
    # 2. Add other .py files in the same directory (e.g. helper modules), skip non-py files
    for file in os.listdir(base_dir):
        if file.endswith('.py') and file != os.path.basename(main_file_path):
            full_path = os.path.join(base_dir, file)
            try:
                with open(full_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                files_list.append({'name': file, 'content': content})
                print(f"📄 Added module: {file}")
            except Exception as e:
...
def main():
    ...
    print("🗑️  Deleting old Main.py from cloud project...")
    try:
        client.delete_file(args.project_id, 'Main.py')
        print("✅ Old Main.py deleted.")
    except Exception as e:
        print(f"⚠️  Could not delete Main.py (may not exist): {e}")
